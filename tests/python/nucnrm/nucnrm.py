...
def checksol(sol, A, B, C = None, d = None, G = None, h = None): 
    ...
    if sol['status'] is 'optimal':
        ...
        print("Dual residual: {}".format(blas.nrm2(res)))

        if m:
           print("Minimum primal slack (scalar inequalities): {}".format(min(h - G*sol['x'])))
           print("Minimum dual slack (scalar inequalities): {}".format(min(sol['z'])))
        ...
        if p:
            ...
            nrmZ = max(s)
            print("Norm of Z: {}".format(nrmZ))
            print("Nuclear norm of A(x) + B: {}".format(nrmX))
            print("Inner product of Z and A(x) + B: {}".format(blas.dot(sol['Z'], X)))
        ...
        base.gemv(G, sol['z'], res, beta = 1.0, trans = 'T')
        print("Dual residual: {}".format(blas.nrm2(res)))
        print("h' * z = {}".format(blas.dot(h, sol['z'])))
        print("Minimum dual slack (scalar inequalities): {}".format(min(sol['z'])))
